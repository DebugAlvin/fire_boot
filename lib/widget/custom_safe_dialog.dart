...
class CustomSafeDialog {
  ...
  static void dismiss<T extends Object?>({
    ...
    List<_CustomSafeDialogRoute> typeStates = (_dialogStates[tag] ??= []);
    if (typeStates.isNotEmpty) {
      try {
        _CustomSafeDialogRoute _safeDialogRoute = typeStates.removeLast();
  ...
  static Future<T?> show<T>({
    ...
    _CustomSafeDialogRoute<T> safeDialogRoute = _CustomSafeDialogRoute<T>(
      context: context,
      builder: builder,
...
class _CustomSafeDialogRoute<T> extends DialogRoute<T> {

  /// 转发 Route 结果
  final Completer<T?> completer = Completer<T?>();

  _CustomSafeDialogRoute({
    required BuildContext context,
    required WidgetBuilder builder,
